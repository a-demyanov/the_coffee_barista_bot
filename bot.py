...
BOT_TOKEN = os.getenv("BOT_TOKEN")  # В Bothost
if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN не задан! Вставь токен в переменные окружения Bothost")

REPORT_CHAT_ID = os.getenv("REPORT_CHAT_ID")  # В Bothost
...
def start_keyboard():
    """Кнопка для начала смены"""
    return ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="☀️ Начать смену")]],
...
async def start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Привет! Давай начнем смену. Нажми кнопку ниже 👇",
        reply_markup=start_keyboard()
    )

# Нажатие кнопки "Начать смену"
@router.message(lambda m: m.text == "☀️ Начать смену")
...
async def dose(message: Message, state: FSMContext):
    ...
    await message.answer("☕ Доза эспрессо (г):")
